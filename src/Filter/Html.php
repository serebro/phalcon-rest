...
class Html {
	...
	public function filter($value) {
		...
		$t = $value;
		$C = array('safe' => 1/*, 'elements'=>'a, b, strong, i, em, li, ol, ul'*/);
		$S = null;

		return $this->_html($t, $C, $S);
	}

	public function _html($t, $C = 1, $S = array()) {
		/*
		...
		*/
			...
			foreach($aA as $k => $v) {
				if (((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e]))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
					...
					if ($k == 'style' && !$C['style_pass']) {
						...
						$v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', function($v){return hl_prot($v);}, $v);
		...
		function htmLawed($t, $C = 1, $S = array()) {
			...
			if ($C['cdata'] or $C['comment']) {
				$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', function($v){return hl_cmtcd($v);}, $t);
			}
			$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', function($v){return hl_ent($v);}, str_replace('&', '&amp;', $t));
			...
			// main
			$t = preg_replace_callback('`<(?:(?:\s|$)|(?:[^>]*(?:>|$)))|>`m', function($v){return hl_tag($v);}, $t);
		...
		function hl_spec($t) {
			// final $spec
			$s = array();
			$x = preg_replace_callback('/"(?>(`.|[^"])*)"/sm', function($m){
				return substr(
					str_replace(
						array(";", "|", "~", " ", ",", "/", "(", ")", '`"'),
